...
import net.codetreats.sevdesk.model.update.OrderUpdate
...
class OrderApi(private val client: SevDeskClient) {
    ...
    fun get(headers: Map<String, String> = emptyMap()): List<Order> =
        client.get<Order>("/Order", headers)
    ...
    fun confirmations(limit: Int = 50): List<Order> =
        client.get<Order>("/Order", mapOf("limit" to "$limit", "orderType" to OrderType.ORDER_CONFIRMATION.abbreviation))
    ...
    fun from(startTime: LocalDateTime, type: OrderType? = null): List<Order> = if (type == null) {
        ...
        client.get<Order>("/Order", mapOf(NO_LIMIT, "startDate" to "${startTime.unixTimestamp()}", "orderType" to type.abbreviation))
    }

    fun add(order: OrderSaveContainer) : OrderContainer =
        client.post("/Order/Factory/saveOrder", body = order);
